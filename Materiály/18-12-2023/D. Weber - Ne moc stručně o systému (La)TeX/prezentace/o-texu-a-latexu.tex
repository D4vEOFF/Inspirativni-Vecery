...
- Položka 2
    ...
    \begin{frame}[t]{Co je \TeX}
        \only<2->{
            \begin{itemize}
                \only<2->{\item Sázecí systém}
                \only<3->{\item Programovatelný (každé makro si lze přizpůsobit)}
                \only<4->{\item Dávkový: \emph{textový soubor $\rightarrow$ grafický výstup}}
                \only<5->{\item Portabilní (nezávislý na verzi souboru)}
            \end{itemize}
        }
    \end{frame}
...
\end{document}            
    ...
    \begin{frame}[fragile]{Sazba matematiky}
        \begin{verbatim}
\left|\bigcup\limits_{i=1}^{n}X_i\right|=
\sum_{\emptyset\neq I\subseteq\{1,2,\dots,n\}}(-1)^{|I|-1}
\left|\bigcap_{i\in I}X_i\right|
        \end{verbatim}
    \end{frame}

    \begin{frame}{Výsledek\dots}
        \[\left|\bigcup\limits_{i=1}^{n}X_i\right|=\sum_{\emptyset\neq I\subseteq\{1,2,\dots,n\}}(-1)^{|I|-1}\left|\bigcap_{i\in I}X_i\right|\]
    \end{frame}

    \section{Porovnání \LaTeX u a Wordu}
    \begin{frame}[t]{Pokud o porovnání \LaTeX u a Wordu}
        \only<2-9>{
            \begin{itemize}
        ...
        \only<10->{
            \begin{itemize}
                \only<10->{\item Pro opakované prvky lze zavést vlastní makro}
                \only<11->{\item Velké množství dostupných extérních balíčků}
                \only<12->{\item Cena}
            \end{itemize}
        }
    \end{frame}
    ...
    \begin{frame}[t]{K čemu se \LaTeX{} hodí\dots}
        \only<2->{
            \begin{itemize}
                \only<2->{\item Matematická/technická sazba}
                \only<3->{\item Výzkumné práce, protokoly}
                \only<4->{\item Závěrečné práce}
                \only<5->{\item Skripta}
                \only<6->{\item Prezentace}
                \only<7->{\item (Automatizované) generování dokumentů}
